...
public class CandleStickChart extends XYChart<String, Number> {
	...
	SimpleDateFormat sdf = new SimpleDateFormat("d MM,EEE HH:mm");
	...
	public CandleStickChart(String title, @SuppressWarnings("exports") CategoryAxis xAxis,
			@SuppressWarnings("exports") NumberAxis yAxis, List<BarData> bars, int maxBarsToDisplay) {
		super(xAxis, yAxis);
		this.xAxis = xAxis;
		this.yAxis = yAxis;
		...
		getStylesheets().add(getClass()
				.getResource("/pl/edu/pw/mini/zpoif/project/binance/main/CandleStickChartStyles.css").toExternalForm());
		...
		for (int seriesIndex = 0; seriesIndex < getData().size(); seriesIndex++) {
			...
			while (iter.hasNext()) {
				...
				if (itemNode instanceof Candle && item.getYValue() != null) {
					...
					candle.updateTooltip(bar.getOpen(), bar.getClose(), bar.getHigh(), bar.getLow(), bar.getVolume());
	...
	protected void updateAxisRange() {
		// For candle stick chart we need to override this method as we need to let the
		// axis know that they need to be able
		// to cover the whole area occupied by the high to low range not just its center
		// data value
	...
	private class Candle extends Group {
		...
		public void updateTooltip(double open, double close, double high, double low, double volume) {
			TooltipContent tooltipContent = (TooltipContent) tooltip.getGraphic();
			tooltipContent.update(open, close, high, low, volume);
		}
	...
	private class TooltipContent extends GridPane {
		...
		private TooltipContent() {
			...
			setConstraints(volume, 0, 4);
			setConstraints(volumeValue, 1, 4);
			getChildren().addAll(open, openValue, close, closeValue, high, highValue, low, lowValue, volume,
					volumeValue);
		}

		public void update(double open, double close, double high, double low, double volume) {
	...
	public void setData(List<BarData> bars) {
		...
		// Clear existing data
		dataSeries.get(0).getData().clear();

		// Add new bars
		for (BarData bar : bars) {
			String label = sdf.format(bar.getDateTime().getTime());
			dataSeries.get(0).getData().add(new XYChart.Data<>(label, bar.getOpen(), bar));
		}
